...
def format_search_results(search_results):
    ...
    for idx, result in enumerate(search_results.results, 1):
        ...
        url = result.url
        published_date = f" (Publisehd: {result.published_date})" if hasattr(result, 'published_date') and result.published_date else ""

        markdown_results += f"**{idx}.** [{title}]({url}){published_date}\n"

        if hasattr(result, 'summary') and result.summary:
            markdown_results += f"> **Sumary:** {result.summary}\n\n"
        else:
            markdown_results += "\n"
